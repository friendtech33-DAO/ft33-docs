...
The Boosted Liquidity Vault is a new friendtech33 utility designed to improve the stability of FTW pairs and establish FTW as a liquidity router for the main pillars of DeFi. By incentivizing third-party liquidity provision using high-quality counter assets, the protocol aims to boost liquidity and minimize volatility.

The mechanism provides a framework for the friendtech33 Treasury to mint FTW directly into liquidity pair vaults against allowlisted assets. The counter-asset will be provided by individual holders and the vault will be incentivized by partner incentives.

## Economic Mechanisms

The Boosted Liquidity Vaults are designed to boost liquidity for FTW pairs and stabilize their exchange rate relative to high-quality counter assets through a self-regulating system. The circulating supply of FTW is dynamically adjusted based on its own price fluctuations, but also with the ones of the counter-asset. This property helps stabilize their exchange rate with a self-regulating system.

When the price of FTW increases relative to the counter-asset (either by FTW appreciation or depreciation of the partner token), some of the FTW that was previously minted into the liquidity pool is released back into circulation. This increase in the circulating supply of FTW should, all else being equal, push the price of FTW back down.

Conversely, if the price of FTW decreases relative to the counter-asset (either by FTW depreciation or appreciation of the partner token), previously circulating FTW will now enter the liquidity pool and the protocol will have a claim on it. This decrease in the circulating supply of FTW should, all else being equal, push the price of FTW back up.
...
2. The counter-asset is valued using oracles and FTW is minted 1:1 against the dollar value of the deposited counter-asset.
3. The newly minted FTW and deposited counter-asset are deployed into a liquidity pool.
...
2.  FTW and counter-asset are withdrawn from the liquidity pool in exchange for the LP receipt tokens.
3.  All the FTW received by the vault is burned.
...
- Facilitating the provision of liquidity for FTW pairs through a more streamlined process.
- Increasing the liquidity of FTW pairs by incentivizing users to provide liquidity using high-quality counter assets.
- Dampening FTW volatility against these high-quality counter-pair assets, helping position FTW as a base asset with great liquidity routing.

### For Partners:

- A more efficient liquidity mining vehicle.
- friendtech33 does not take a portion of the rewards provided by the partner protocol, meaning partners get 2x TVL for their rewards compared to traditional liquidity mining systems.
- Since FTW is a volatile asset with decent price stability, using it as a pair provides the following benefits for partners:
...
- Double the rewards (compared to traditional pools) because friendtech33 is forfeiting all of its rewards and allocating them to users instead.

## Risks by Stakeholder

### For the Protocol:

- The FTW is exposed to the counter asset's volatility.
- Newly minted supply could lead to FTW depreciation if the market is inefficient .

### For Partners:

- The asset is exposed to FTW's volatility.
- If FTW depreciates excessively in price, their asset could be impacted due to arbitrageurs. In such scenario, arbitrageurs would buy "cheap" FTW from the main friendtech33' pools, arb the BLV pool, and realize the profits by selling the acquired partner token to their main pool. However, this risk is minimized by the fact that RBS (Range Bound Stability) is actively dampening FTW volatility versus DAI, and the fact that the Treasury provides an infinite bid at liquid backing.
...
- To encourage users to provide liquidity and reduce the risks associated with liquidity provision, friendtech33 is giving up all of its rewards and allocating them to users instead.
- To mitigate oracle attacks, there is a 24-hour withdraw period from time of last deposit.

By participating in the Boosted Liquidity Vaults, users and partners can take advantage of the benefits offered by friendtech33 DAO while managing their risks. We believe this mechanism will play an important role in the growth and stability of the friendtech33 ecosystem.

## Security Considerations

Security is of utmost importance to friendtech33. The DAO is committed to ensuring that the participants of the econohmy are always safe and secure. With this commitment, the Boosted Liquidity Vaults implement several measures to mitigate security risks.
...
Secondly, the vault contracts are designed in a manner where user deposits are recorded as liabilities, thus preventing any commingling of funds. This procedure guarantees that the underlying funds can solely be accessed by the depositor and not by external entities, including friendtech33.
...
friendtech33 takes the security of its ecosystem very seriously and is committed to ensuring that its users' assets are always protected. With these measures in place, participants can have confidence in the security of the Boosted Liquidity Vaults.
...
### deposit
Allows users to deposit a certain amount of the pair tokens. Mints the equivalent FTW amount in USD, and creates a liquidity position with both tokens. The vault keeps ownership of the LP tokens and tracks the relative ownership of the depositor.
```
...
### withdraw
Allows users to receive back the corresponding amount pair tokens based on the current composition of the liquidity pool. The vault removes the given amount of LP tokens, returns any received pair tokens to the user, and burns the received FTW.
...
/// @param  minTokenAmounts_    The minimum amounts of pair tokens and FTW to receive
...
### claimRewards
At any given time, users have the ability to claim the rewards they have earned. Both external reward tokens (incentives provided by the partner) and internal rewards tokens (any incentives that friendtech33 may provide) are claimed when calling this function.
```
...
Virtual functions are not implemented by the `SingleSidedLiquidityVault` contract. Instead, those functions must be implemented by the developers which aim to integrate a new SSLV on top of friendtech33.
These are the virtual functions which must be overrriden by the implementation contract:

- `_valueCollateral:` Internal function which calculates the equivalent FTW amount for a given amount of partner tokens. This function will usually use several price feeds to calculate the conversion rate.
```            
/// @param amount_      The amount of partner tokens to calculate the FTW value of
/// @return ohmAmount   The amount of FTW for the given amount of partner tokens
function _valueCollateral(uint256 amount_) internal view virtual returns (uint256 ohmAmount);
```

- `_getPoolPrice:` Internal function which calculates the current price of the liquidity pool and expresses it in FTW/TKN ratio.
```
/// @return ohmTknRatio  The current price of the liquidity pool in FTW/TKN
function _getPoolPrice() internal view virtual returns (uint256 ohmTknRatio);
```

- `_getPoolOhmShare:` Internal function which calculates the vaults' current share of FTW in the liquidity pool.
```
/// @return ohmShare   The contract's current share of FTW in the liquidity pool
function _getPoolOhmShare() internal view virtual returns (uint256 ohmShare);
```

- `_deposit:` Internal function which deposits FTW and partner tokens into the liquidity pool. This functions is called by the core function `deposit` which users use to interact with the vault. This function should also handle deposits into any external staking pools like Aura or Convex.
```
/// @param ohmAmount_    The amount of FTW to deposit
...
- `_withdraw:` Internal function which withdraws FTW and partner tokens from the liquidity pool. This functions is called by the core function `withdraw` which users use to interact with the vault. This function should also handle withdrawals from any external staking pools like Aura or Convex.
```
/// @param lpAmount_         The amount of liquidity pool tokens to withdraw
/// @param minTokenAmounts_  The minimum amounts of FTW and partner tokens to receive
/// @return ohmOut           The amount of FTW received
...
3. Approve the core Balancer Vault as spender of both tokens (FTW and the pair token).
...
1. Cast the liquidity pool address from abstract to Balancer Base pool.
2. Fetch the underlying naked assets (FTW and the pair token) held by the vault.
3. Unstake the BPT tokens from Aura.
4. Make a request to the Balancer Vault to leave the liquidity pool.
5. Return the amount of extra tokens (FTW and the pair token) held in the vault.
...
1. Fetch the necessary price feed oracles to get the ratio of `FTW / PairToken`. Make sure that the decimals are in place.
2. Return the conversion of the input `_amount` of pair token in FTW terms.

### getPoolPrice

When implementing the `_getPoolPrice` function:
1. Calculate the `FTW / pairToken` ratio of the Liquidity pool by fetching the amount of each token in the pool.
